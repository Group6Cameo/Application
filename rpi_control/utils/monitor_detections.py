...
class RecordManager:
    def __init__(self, max_records=200, max_age_seconds=5.0):
        # each item is (datetime_obj, row_data_list)
...
def run_bash_script():
    ...
    try:
        script_path = os.path.join(CURRENT_DIR, 'face_recognition.sh')
    ...
    try:
        if "HailoROI" in data:
            if "HailoBBox" in data["HailoROI"]:
                ...
                face_info['center_x'], face_info['center_y'] = calculate_center(
                    bbox)
...
def monitor_zmq():
    ...
    record_manager = RecordManager(
        max_records=MAX_ROWS, max_age_seconds=MAX_HISTORY_SECONDS)
                ...
                if socket.poll(100):
                    ...
                    if data_id not in processed_data_ids:
                        ...
                        if face_info['mode0_id'] is not None:
                            ...
                            if gallery_id != 'nd':
                                new_unique_ids = set(
                                    r[3] for _, r in record_manager.records if r[3] != 'nd')
                                if new_unique_ids != current_unique_ids:
                                    current_unique_ids = new_unique_ids
                                    if current_unique_ids:
                                        face_ids_socket.send_string(
                                            json.dumps(list(current_unique_ids)))
